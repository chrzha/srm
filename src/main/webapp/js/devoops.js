...
function LoadDataTablesScripts2(callback){
	function LoadDatatables2(){
		$.getScript('plugins/datatable1_1/jquery.dataTables.js', function(){
					$.getScript('plugins/datatables/dataTables.bootstrap.js', callback);
				});  
	}
	if (!$.fn.dataTables){
...
function LoadAjaxContentById(url,elementId){
	$('.preloader').show();
	$.ajax({
		mimeType: 'text/html; charset=utf-8', // ! Need set mimeType only when run from local file
		url: url,
...
$(document).ready(function () {
	...
	if (ajax_url.length < 1) {
		ajax_url = 'material/view/main';//TODO  index ajax page
	}
